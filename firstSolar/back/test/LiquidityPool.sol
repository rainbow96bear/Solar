...
contract LiquidityPool {
  ...
  function swapTokens(
    ...
    //ERC-20 토큰을 "_tokenIn"에서 이 함수를 호출한 사용자의 주소인 "msg.sender"에서 이 컨트랙트 계좌의 주소인 "address(this)"로 전송하는 코드입니다.
    ...
    // _amountInWithFee는 유동성 교환에 참여하는 실제 자산의 양이며, 이후 코드에서 유동성 풀의 상태를 업데이트하고, 교환할 자산의 양을 계산하는 데 사용됩
    /*
    ...
    // token1과 token2는 각각 IERC20 인터페이스를 구현한 토큰 계약을 가리키는 변수입니다.
  ...
  function addLiquidity(
    ...
    // token1과 token2의 잔액인 reserve2 ,reserve2를 사용하여 수량과 교환 비율을 유지하는지 확인
    //예를 들어, "_amountToken1"이 100이고 "_reserve1"이 200, "_amountToken2"가 150이고 "_reserve2"가 300일 경우, 위 수식은 다음과 같이 계산됩니다.
    // _amountToken1 * _reserve2 = 100 * 300 = 30,000
    // _amountToken2 * _reserve1 = 150 * 200 = 30,000
    ...
    //교환 비율이 유지되지 않는 상황에서의 교환을 방지할 수 있습니다.
